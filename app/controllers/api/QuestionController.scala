...
class QuestionController @Inject()(cc: ControllerComponents)(qs: QuestionService)(
  ...
  def postQuestion() = Action(parse.json).async { request =>
    ...
    createId
      .map(x => QuestionResult(SaQuestionModel(id = Some(x)))) // TODO 作成した質問のIDを返すようにする
  ...
  def putQuestion() = Action(parse.json).async { request =>
    ...
    updateCount
      .map(x => QuestionResult(SaQuestionModel(id = Some(x)))) // TODO 処理件数を返すようにする
      .map(y => Ok(Json.toJson(y)))
  }

  def getAllQuestion() = Action.async {
    qs.readQuestion(None, None).map(x => QuestionListResult(x)).map(y => Ok(Json.toJson(y)))
  }
